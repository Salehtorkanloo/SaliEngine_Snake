...
void SnakePlayer::load(int x, int y){
    ...
    int gridWidth = TheGame::instance()->getGrid()->getGridWidth() - 1;
    if(initSnakeSize > gridWidth)
        initSnakeSize = gridWidth;
    moveDir = Vector2D::right();
    //set body to 4
    grow(initSnakeSize);
...
void SnakePlayer::update(){
    ...
    for(int i = 0 ; i < g_body_positions.size(); ++i){
        if(drawInitializeSnakeBody)
        {
            const Vector2D headPos = g_head_position.getPositionInGridPosition();
            g_body_positions[i].setPositionFromGridPosition(headPos - moveDir * i);
            std::cout << "bodyPos: " << g_body_positions[i].getPositionInGridPosition() << "\n";
            continue;
        }
...
void SnakePlayer::updateSnakeBodyPosition(){
