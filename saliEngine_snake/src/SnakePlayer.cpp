...
void SnakePlayer::update(){
    ...
    for(int i = 0 ; i < g_body_positions.size(); ++i){
        ...
        {
            const Vector2D& drawingDir = -moveDir;
            const Vector2D bodyPos = g_body_positions[i].getPositionInGridPosition();
            g_body_positions[i].setPositionFromGridPosition(bodyPos + drawingDir * g_width);
            return;
        }
